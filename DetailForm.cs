...
{
    /// <summary>
    /// Reusable page for a single section of a specific profile (Box1..Box4).
    /// It reads/writes section content using (username, profileKey, section).
    /// </summary>
    public class DetailForm : Form
    {
        protected readonly AboutMeForm _aboutMe;
        protected readonly string _username;
        protected readonly string _profileKey; // e.g., "Box1"
        protected readonly string _section;    // e.g., "Education"

        // Initialize immediately to satisfy non-nullability warnings
        private Label _bodyLabel = new Label();

        public DetailForm(AboutMeForm aboutMe, string username, string profileKey, string pageTitle, string sectionKey)
        {
            Theme.Apply(this);
            ...
            Text = $"{pageTitle} — {_profileKey}";
            StartPosition = FormStartPosition.CenterScreen;
            ClientSize = new Size(1000, 640);
            MinimumSize = new Size(820, 540);

            // --- Load content for this (username, profileKey, section) ---
            string body;
            string photoPath;
            using (var db = new AppDb())
            {
                // REQUIRED: pass the section key as 3rd parameter
                body = db.GetSection(_username, _profileKey, _section);
                photoPath = db.GetProfile(_username, _profileKey).PhotoPath;
            }
            ...
            _bodyLabel = Theme.BodyText(body); // now definitely initialized
        ...
        private void BtnEdit_Click(object? sender, EventArgs e)
        {
            ...
            var currentBody = db.GetSection(_username, _profileKey, _section);

            using var dlg = new EditDialog($"Edit {_section}", currentBody);
            if (dlg.ShowDialog(this) == DialogResult.OK)
            {
                var newBody = dlg.EditedText ?? string.Empty;

                // REQUIRED: Pass the body as the 4th parameter
                db.SaveSection(_username, _profileKey, _section, newBody);
                _bodyLabel.Text = newBody;
            }
        }
